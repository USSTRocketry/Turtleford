...
{
    std::array<std::byte, 10> Part1; Part1.fill(std::byte{0x11});
    std::array<std::byte, 10> Part2; Part2.fill(std::byte{0x22});
...
TEST_F(LoggerTest, LogInfoEncodesTimestampSeverityAndMessage)
{
    ...
    const auto DecodedMsg = std::unique_ptr<std::string>(
...
TEST_F(LoggerTest, LogInfoSeverityRoundTripAllLevels)
{
    ...
    for (size_t i = 0; i < Levels.size(); ++i)
    {
        ...
        ra::Logger::LogInfo Info {
            ...
            .Level     = Levels[i],
            .Status    = static_cast<uint32_t>(200u + i),
            .Msg       = Msg,
        };
